...
<div class="card card-primary">
    ...
    <div class="card-body">
        ...
        <table id="example5" class="table table-bordered table-striped">
            ...
            foreach ($dt as $det) {
                ...
                if ( $det->status == "Belum") {
                    $bg = "bg-secondary";
                }elseif($det->status == "KKI"){
                    $bg = "bg-info";
                }elseif($det->status == "Kerja"){
                    $bg = "bg-primary";
                }else{
                    $bg = "bg-danger";
                }
                ...
                if ($det->hasil == "Belum") {
                    $h = "bg-secondary";
                }elseif($det->hasil == "Cancel") {
                    $h = "bg-danger";
                }elseif($det->hasil == "Diterima KKI") {
                    $h = "bg-info";
                }elseif($det->hasil == "Lolos Test") {
                    $h = "bg-primary";
                }elseif($det->hasil == "Gagal Test") {
                    $h = "bg-danger";
                }elseif($det->hasil == "Menolak") {
                    $h = "bg-danger";
                }else{
                    $h = "bg-danger";
                }
            ?>
                    ...
                    <tr>
                        <td><a href="<?= base_url('') ?>master/update_dt?id_permintaan=<?= $det->id_permintaan ?>&nim=<?= $det->nim ?>"><?= $det->nim ?></a></td>
                        <td><?= $det->nama_mhs ?></td>
                        <td><?= $det->kelas ?></td>
                        <td><?= $det->prodi ?></td>
                        <td><span class="badge <?= $h ?>"><?= $det->hasil ?></span></td>
                        <td><span class="badge <?= $bg ?>"><?= $det->status ?></span></td>
                        <td><?= $det->ket_lain ?></td>
                    </tr>
    ...
    <div class="card-footer">
        ...
        <a class="btn btn-primary btn-sm" href="<?= base_url('') ?>master/tambah_kandidat?id_permintaan=<?= $det->id_permintaan ?>">
        ...
        <a onclick="return ubah(`<?= $ht->id_permintaan ?>`,`<?= $ht->id_perusahaan ?>`, `<?= $ht->bidang ?>`, `<?= $ht->relasi ?>`, `<?= $ht->alamat ?>`, `<?= $ht->kontak_person ?>`, `<?= $ht->jabatan ?>`, `<?= $ht->no_telp ?>`,`<?= $ht->posisi_yang_dibutuhkan ?>`, `<?= $ht->kualifikasi ?>`)" class="btn btn-primary btn-sm">
            <i class="fas fa-edit"></i><span style="color:#fff;"> Ubah</span>
        </a>
    </div>
</div>
...
<script>
    ...
    function ubah(id_permintaan, nama_perusahaan, bidang, relasi, alamat, kontak_person, jabatan, no_telp, posisi_yang_dibutuhkan, kualifikasi) {
        ...
        document.form.action = '<?= base_url('Master/permintaan_update_aksi'); ?>';
    }
    ...
    function hapus(id_permintaan, nama_perusahaan) {
        ...
        document.form.action = '<?= base_url('Master/permintaan_delete'); ?>';
    }
</script>